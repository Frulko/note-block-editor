...
- **Native Swift: the model half is verified** (`native/swift`, 2026-08-07).
  ...
  idea of the format.
  ~~`loro-swift`~~ — done 2026-08-08. `NbeSync` (a separate target, so
  `NbeModel` keeps its zero dependencies) opens a **CRDT snapshot exported by
  `packages/collab`**, with the same blocks, types, ids and text. The two
  implementations now agree on the merge format, not only on the JSON. The
  fixture is generated by the editor's own store with a fixed peer id, so it is
  provably current and stable across runs.
  ~~Swift as a peer~~ — done 2026-08-08. `DocumentWriter` creates blocks in the
  same layout, and the round trip is checked by *both* implementations rather
